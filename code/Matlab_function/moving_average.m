function [x_result] = moving_average(x,win_size,mv_size,start_end)
...
if start_end == 1
    for i=1:ceil((size(x,1)-win_size+1)/mv_size)
        x_result(i,:) = mean(x(1+(i-1)*mv_size:1+(i-1)*mv_size+win_size-1,:),1);
    end
elseif start_end == 2
    for i=1:ceil((size(x,1)-win_size+1)/mv_size)
        x_result(end-i+1,:) = mean(x(end-(i-1)*mv_size-win_size+1:end-(i-1)*mv_size,:),1);
    end
end
